...
):RecyclerView.Adapter<UserViewAdapter.UserViewSlipsViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserViewSlipsViewHolder {
        return UserViewSlipsViewHolder(SlipsItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }

    override fun getItemCount(): Int = slipItems.size

    override fun onBindViewHolder(holder: UserViewSlipsViewHolder, position: Int) {
        holder.bind(position)
    }

    inner class UserViewSlipsViewHolder(private val binding:SlipsItemBinding):RecyclerView.ViewHolder(binding.root) {
            init {
